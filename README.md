...
1. 点击顶部“选择演示采购记录”，选择包含 `Sheet1` 的本地脱敏工作簿，再生成目标数据表或透视表。
...
- 演示工作簿可在运行任务前显式选择，避免首次操作时不清楚应使用哪个本地文件。
- 生成结果按 `Sheet1 → 合成品类数据表 → 对应透视表` 的顺序排列，使数据源和结果保持相邻。
- 原生透视表使用明确的连续数据区域创建缓存，不依赖 Excel 当前选中区域；失败提示会标明创建阶段。
- 浏览器会话保存到当前 Windows 用户的本地应用数据目录，并绑定电脑与用户名，项目文件夹复制到其他同事电脑时不会带走登录状态。
- 补查优先依据待回填清单识别确实缺少工作分解结构的凭证，避免旧工作簿的空白字段扩大为全量重跑。
- 项目名称解析保留 `+`、`.`、`/`、`&`、`#` 等有效字符，并在下一个字段开始前停止读取。
...
公开版离线测试覆盖合成品类配置、`DEMO-MAT-*` 物料筛选、生成 Sheet 排列、透视表连续数据源和配置模板校验。它们使用临时文件和模拟 Excel 对象；不访问门户、不启动浏览器，也不等同于在真实业务环境中的端到端验收。
